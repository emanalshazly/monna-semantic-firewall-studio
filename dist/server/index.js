...
const candidateFromModel=async(question,env)=>{
  if(!env.OPENAI_API_KEY){
    ...
    if(normalized.includes("maximum rpm")||normalized.includes("max rpm")) return {candidate:{intent:"QUERY",entry_class:"Centrifuge",steps:[{edge_id:"maxRPM",target:"xsd:int"}],ambiguous:false,summary:"Public demo fixture for the grounded path."},model:"gpt-5.6-demo-fixture",response_id:null};
    if(normalized.includes("add governedby")) return {candidate:{intent:"ONTOLOGY_CHANGE_REQUEST",entry_class:"Spectrometer",steps:[],ambiguous:false,summary:"Public demo fixture for a governed change request."},model:"gpt-5.6-demo-fixture",response_id:null};
    if(normalized.includes("spectrometer")&&normalized.includes("protocol")) return {candidate:{intent:"QUERY",entry_class:"Spectrometer",steps:[{edge_id:"governedBy",target:"Protocol"}],ambiguous:false,summary:"Public demo fixture for an unsupported relationship."},model:"gpt-5.6-demo-fixture",response_id:null};
    throw new Error("DEMO_FIXTURE_ONLY");
  }
